  ...
  if (pelotitaChocaNave & pelotitaEnAncho) {
    ...
    if (lives <= 0) {
      Swal.fire({
        icon: "error",
        title: "¡Perdiste!",
        text: `Te quedaste sin vidas, ${playerName}.`,
        confirmButtonText: "Reintentar",
      }).then(() => {
        solicitarNombreJugador(); // Solicita el nombre y reinicia el juego
      });
      gameRunning = false; // Pausa el juego
    } else {
      Swal.fire({
        icon: "error",
        title: `¡PERDISTE, ${playerName}!`,
...
function solicitarNombreJugador() {
  Swal.fire({
    ...
    resetJuego(); // Reinicia el juego después de ingresar el nombre
  });
}
